...
export const ATTENDANCE_OUTCOMES: Array<{ key: AttendanceOutcome; label: string }> = [
  { key: 'awaiting_response', label: 'Aguardando resposta' },
  { key: 'proposal_sent', label: 'Proposta enviada' },
  { key: 'negotiation', label: 'Em negociação' },
  { key: 'client_replied', label: 'Cliente respondeu' },
  { key: 'waiting_internal_quote', label: 'Aguardando orçamento interno' },
  { key: 'sale_closed', label: 'Venda fechada' },
  { key: 'post_sale_done', label: 'Pós-venda realizado' },
  { key: 'invalid_phone', label: 'Telefone inválido' },
  { key: 'no_interest', label: 'Sem interesse' },
  { key: 'record_only', label: 'Apenas registrar' },
];
...
const CONFIG: Record<AttendanceOutcome, {
  ...
  awaiting_response: { label: 'Atendimento realizado — aguardando resposta', stage: 'contato_realizado', action: 'Verificar resposta do cliente', days: 2, attendanceState: 'aguardando_cliente', eventCode: CRM_EVENT_CODES.CONTACT_ATTEMPTED },
  proposal_sent: { label: 'Proposta enviada', stage: 'proposta_enviada', action: 'Fazer follow-up da proposta', days: 2, attendanceState: 'aguardando_cliente', eventCode: CRM_EVENT_CODES.PROPOSAL_SENT },
  negotiation: { label: 'Cliente em negociação', stage: 'negociacao', action: 'Retomar negociação', days: 1, attendanceState: 'aguardando_cliente', eventCode: CRM_EVENT_CODES.NEGOTIATION_STARTED },
...
export async function applyAttendanceOutcome(input: {
  ...
  const nextStage = requestedStage === 'contato_realizado'
    ? (['novo_lead', 'cadencia'].includes(currentStage) ? 'contato_realizado' : currentStage)
    : requestedStage;
  const returnAt = dueInDays(config.days);

  const { error: updateError } = await supabase.from('contacts').update({
    ultimo_contato: now.slice(0, 10),
    funnel_status: nextStage,
    next_action_text: config.action ?? null,
    next_action_date: returnAt,
    next_contact_date: returnAt,
    updated_at: now,
  }).eq('id', input.contactId);
  ...
  if (!conversationId) {
    const { data } = await supabase.from('service_conversations').select('id')
      .eq('contact_id', input.contactId).order('last_message_at', { ascending: false }).limit(1).maybeSingle();
    conversationId = data?.id ?? null;
  }
  if (conversationId) {
    const { error } = await supabase.from('service_conversations').update({
      attendance_state: config.attendanceState,
      status: config.resolved ? 'resolved' : 'open',
      resolved_at: config.resolved ? now : null,
      needs_reply: false,
      unread_count: 0,
      return_at: returnAt,
  ...
  await syncCrmNextActionTask(input.contactId, { title: config.action, dueAt: returnAt });
  return { label: config.label, nextStage, returnAt, attendanceState: config.attendanceState };
}

export async function snoozeAttendance(input: {
  contactId: string;
  conversationId?: string | null;
  when: number | string;
}) {
  const target = typeof input.when === 'number' ? new Date() : new Date(`${input.when}T09:00:00`);
  if (typeof input.when === 'number') {
    target.setDate(target.getDate() + input.when);
    target.setHours(9, 0, 0, 0);
  }
  if (Number.isNaN(target.getTime())) throw new Error('Data inválida');
  const returnAt = target.toISOString();
  const title = 'Retomar atendimento';

  const { error: contactError } = await supabase.from('contacts').update({
    next_action_text: title, next_action_date: returnAt, next_contact_date: returnAt, updated_at: new Date().toISOString(),
  }).eq('id', input.contactId);
  if (contactError) throw contactError;

  if (input.conversationId) {
    const { error } = await supabase.from('service_conversations').update({
      attendance_state: 'retornar_em', return_at: returnAt, needs_reply: false,
    }).eq('id', input.conversationId);
    if (error) throw error;
  }

  const { error: historyError } = await supabase.from('contact_history').insert({
    contact_id: input.contactId, event_type: 'follow_up', interaction_type: 'follow_up',
    event_code: CRM_EVENT_CODES.FOLLOW_UP_SCHEDULED,
    event_metadata: { source: 'unified_inbox', return_at: returnAt },
    description: `Atendimento adiado para ${target.toLocaleString('pt-BR')}`,
    interaction_date: new Date().toISOString(),
  });
  if (historyError) throw historyError;
  await syncCrmNextActionTask(input.contactId, { title, dueAt: returnAt });
  return returnAt;
}
