...
interface Props {
  ...
  platforms: Platform[];
  onSelectVariation: (v: DigitalVariation) => void;
}
...
const COLUMNS: ColDef[] = [
  ...
  { key: 'order', label: '#', default: 60, min: 50 },
  { key: 'platform', label: 'Plataforma', default: 170, min: 120 },
  { key: 'title', label: 'Título', default: 260, min: 160 },
...
const STORAGE_KEY = 'digital:variations-spreadsheet:widths:v1';

export function VariationsSpreadsheetView({ idea, variations, platforms, onSelectVariation }: Props) {
  ...
  const getCover = (v: DigitalVariation): string | null => {
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      const getVal = (v: DigitalVariation): string | number => {
        switch (sortKey) {
          case 'order':
            return v.order_index ?? 0;
          case 'platform':
    ...
    return arr;
  }, [variations, sortKey, sortDir, platformsMap]);
