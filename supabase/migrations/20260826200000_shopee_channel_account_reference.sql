-- FASE 3C: Operações/Shopee passa a registrar a Conta/Loja canônica.
-- Os textos legados marketplace_account permanecem para compatibilidade e histórico.
ALTER TABLE public.orders
  ADD COLUMN channel_account_id uuid REFERENCES public.channel_accounts(id) ON DELETE RESTRICT;

CREATE INDEX orders_channel_account_id_idx
  ON public.orders (channel_account_id);

ALTER TABLE public.marketplace_product_mappings
  ADD COLUMN channel_account_id uuid REFERENCES public.channel_accounts(id) ON DELETE RESTRICT;

CREATE INDEX marketplace_product_mappings_channel_account_id_idx
  ON public.marketplace_product_mappings (channel_account_id);

CREATE UNIQUE INDEX marketplace_product_mappings_canonical_account_item_unique_idx
  ON public.marketplace_product_mappings (marketplace, channel_account_id, external_item_key)
...
BEGIN
  ...
  -- A nova interface envia o ID. A resolução textual só preserva clientes ainda não sincronizados.
  ...
  ELSE
    -- Não confia no texto enviado pelo navegador: valida a conta e a plataforma pela referência canônica.
  ...
  -- Only complete, unmasked identifiers may link an existing contact.
  ...
  -- A recipient with no strong match becomes an operational customer only.
  -- Buyer username remains on the order metadata, never as the contact name.
  ...
  INSERT INTO public.orders (
    ...
    channel_account_id, marketplace_account, status, total_value, order_date, due_date,
    delivery_date, discount_amount, shipping_amount, notes, order_type,
    delivery_snapshot, marketplace_metadata
  ) VALUES (
    v_external_order_id,
    COALESCE(v_recipient_name, NULLIF(p_order->>'buyer_username', '')),
    NULLIF(v_phone_raw, ''), v_contact_id, 'shopee', v_channel_account_id, v_account, 'enviado', v_total,
